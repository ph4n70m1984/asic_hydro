...
-   🧠 **FreeRTOS Dual-Core Architecture:**
    ...
    -   **Core 1 (Real-Time Control):** Изолированная задача `TaskControl` --- строго детерминированный контур реального времени (100mc / `vTaskDelayUntil`). Расчет ПИД, Slew Rate Limiter ЦАПа, асинхронный опрос датчиков и FSM Антизалипания.
...
-   🌡️ **Неблокирующий OneWire (DS18B20):** Асинхронный опрос температуры с разделением старта конвертации и считывания (`setWaitForConversion(false)`). Полностью устранены задержки в 750mc.
...
-   🔒 **NVS & MQTT Protection:**
    ...
    -   Отклонение сбойных/дефолтных уставок 20.0 °C при подключении к брокеру.
...
-   **Два профиля управления выходом ЦАП (GPIO26):**
    ...
    -   **Заслонка/Кран (4-20mA): Масштабирование 0% -> 4.00 mA 100% -> 20.00 mA

    -   **Сухая градирня (0-10v): Масштабирование 0% -> 0.00 v 100% -> 10.00 v

-   **Надежность промышленных ПЛК:**

    -   Аппаратный Watchdog (`esp_task_wdt`) с таймаутом 30 сек.

    -   Безопасный старт (Safe Boot): принудительное удерживание ЦАП в 0% (4.00 mA/0.00 v) при включении питания.
...
-   **Локальный ПИД-регулятор (`QuickPID`):**
    ...
    -   Дискретность расчета: 5 секунд, настраиваемая мертвая зона (Deadband 2.0%).

    -   Диапазон уставки **T_out**: **21.0°C - 85.0°C**.

    -   Фильтрация шумов DS18B20 через **Exponential Moving Average (EMA)** a = 0.08).

-   **Плавный ход (Slew Rate Limiter):**

    -   Программный демпфер с шагом 0.2% за цикл (100 мс) для защиты механики и предотвращения гидроударов.

-   **Антизалипание механики (Anti-Stuck FSM):**

    -   Автоматический 24-часовой суточный цикл полной отработки заслонки (0% -> 100% -> 0%) для предотвращения заклинивания.
...
-   **Мониторинг периферии:**
    ...
    -   Две независимые шины OneWire (DS18B20) для температур на входе (**T_in**) и выходе (**T_out**).
